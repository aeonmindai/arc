...
| **TurboQuant KV** *(paged default at head_dim 128)* | Zandieh et al. (Google Research, [arXiv:2504.19874](https://arxiv.org/abs/2504.19874), ICLR'26) — Arc Rust implementation of WHT + Lloyd-Max KV coding. **On by default on the paged path**: a standard-layout head_dim-128 model on CUDA gets K4/V3 KV with no flag, and loses prefix caching with it (`--pa-cache-type auto` opts out). The kernels are now instantiated at head_dim 64/128/256/512, but only 128 has ever executed on hardware and only 128 is reachable without asking — every other geometry falls back to `auto` with a warning, so DeepSeek V4's head_dim of 512 is opt-in via an explicit `--pa-cache-type turboquant`. Served end-to-end on a B200 at **55 tok/s with correct output** at head_dim 128; **quality has never been evaluated**, at any width. Eager (non-paged) KV is a separate path, opt-in via `ARC_TURBOQUANT_KV=1`. |
...
What *has* run: the TurboQuant K4/V3 paged path served Qwen3-32B on a B200 end-to-end with correct output (see [Results](#results)) — so "no TurboQuant forward pass has ever been benchmarked", which this section previously asserted, was false. What that run does **not** establish is any of the ratios above, or any quality number, and it was at head_dim 128 only. TurboQuant is the paged default at head_dim 128; the paged kernels now also cover V4's head_dim of 512, but nothing has ever run at that width, so it stays opt-in. The xKV pool is not implemented.
...
>
> Off that envelope the default falls back to `Auto` with a warning: MLA layouts, uninstantiated head dims, and the instantiated-but-unmeasured widths 64/256/512. Those three are reachable only by asking for them — an explicit `--pa-cache-type turboquant` is accepted at any instantiated width, and is a hard error off that set. The eager (non-paged) KV path is separate and genuinely opt-in via `ARC_TURBOQUANT_KV=1`.
>
> **Provenance:** this path has served a real model on real hardware — Qwen3-32B, B200, 55 tok/s, correct output — but at b=1, **head_dim 128 only**, one preset, with **no quality evaluation and no A/B against an unquantized cache**. Widths 64/256/512 compile; none has ever executed. See [Results](#results) before relying on it.
