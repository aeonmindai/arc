...
- [ ] **Criterion 1** — V4 Flash decodes a coherent paragraph (`arc run -m deepseek-ai/DeepSeek-V4-Flash -a deepseekv4 --isq qtip2 --pa-cache-type turboquant`). `--isq qtip2` is mandatory — V4 Flash is a multi-hundred-billion-param MoE (hundreds of GB at bf16) and OOMs one 80 GB H100 without ISQ. (Exact param count is disputed in-repo — 140B in `validate.rs` vs 284B in README/QTIP comments/research; Neo to reconcile, see overnight report. The OOM point holds at either.)
