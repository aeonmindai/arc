...
# ---------------- GSM8K train (8-shot exemplars, session 4) ----------------
# Source of the run_gsm8k.py --eight-shot exemplar pool. Best-effort: only the
# --eight-shot mode needs it, so a download failure WARNS instead of failing
# the whole fetch.
GSM_TRAIN_OUT="data/gsm8k_train.jsonl"
GSM_TRAIN_RAW="https://raw.githubusercontent.com/openai/grade-school-math/master/grade_school_math/data/train.jsonl"
if [ -s "$GSM_TRAIN_OUT" ]; then
  ok "$GSM_TRAIN_OUT already present ($(wc -l < "$GSM_TRAIN_OUT") lines)"
else
  if curl -fsSL --retry 3 -o "$GSM_TRAIN_OUT.part" "$GSM_TRAIN_RAW"; then
    mv "$GSM_TRAIN_OUT.part" "$GSM_TRAIN_OUT"
    TRAIN_LINES=$(wc -l < "$GSM_TRAIN_OUT")
    [ "$TRAIN_LINES" -ge 7000 ] || fail "gsm8k_train.jsonl has only $TRAIN_LINES lines (expect 7473)"
    head -1 "$GSM_TRAIN_OUT" | grep -q '####' || fail "gsm8k_train.jsonl first record lacks '####' marker"
    ok "GSM8K train set ready ($TRAIN_LINES problems)"
  else
    echo "WARN: GSM8K train download failed — --eight-shot mode unavailable (0-shot + voting unaffected)"
  fi
fi
