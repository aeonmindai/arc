...
// Fused gate-and-up GEMV with silu*mul folded into the output write.
// Each block (1 warp = 32 threads) handles ONE pair of rows: it computes
//   gate_acc = sum(W_gate[m] * input)
//   up_acc   = sum(W_up[m]   * input)
//   output[m] = silu(gate_acc) * up_acc
// in registers, never materializing the full gate / up vectors. Eliminates
...
extern "C" void arc_launch_gemv_bf16_dual_silu_mul(
