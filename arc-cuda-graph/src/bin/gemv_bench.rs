...
use arc_cuda_graph::gemv_ffi::{
    arc_launch_gemv_bf16, arc_launch_gemv_bf16_clocked, arc_launch_gemv_bf16_dual,
    arc_launch_gemv_bf16_dual_silu_mul, arc_launch_gemv_bf16_dual_silu_mul_2x16,
    arc_launch_gemv_orig_16x2, arc_launch_gemv_orig_16x3, arc_launch_gemv_orig_1x16,
    arc_launch_gemv_orig_1x32, arc_launch_gemv_orig_2x16, arc_launch_gemv_orig_2x24,
    arc_launch_gemv_orig_2x32, arc_launch_gemv_orig_4x12, arc_launch_gemv_orig_4x16,
    arc_launch_gemv_orig_4x8, arc_launch_gemv_orig_8x4, arc_launch_gemv_orig_8x6,
    arc_launch_gemv_orig_8x8, arc_launch_gemv_orig_pipe,
};
use arc_cuda_graph::nongemv_ffi::{
    launch_fused_rmsnorm_residual_bf16_clocked, launch_rmsnorm_qk_pair_bf16_clocked,
    turbo_paged_attention_v1_bf16out_clocked, turbo_reshape_and_cache_clocked,
};

extern "C" {
    fn cudaMalloc(ptr: *mut u64, size: usize) -> u32;
    fn cudaFree(ptr: u64) -> u32;
    fn cudaMemcpy(dst: *mut c_void, src: *const c_void, count: usize, kind: u32) -> u32;
...
fn print_phases(name: &str, phases: &[(&str, Vec<u64>)]) {
    ...
    for (label, vec) in phases {
        ...
        println!(
            "    {:>14}  med={:7}  p10={:7}  p90={:7}  ({:5.1}% of total)",
            label, med, p10, p90, pct
        );
    }
}
...
unsafe fn bench_fused_rmsnorm_residual(hidden_size: i32, batch_size: i32, with_residual: bool) {
    ...
    if with_residual {
        fill_random_bf16(residual, elem);
    }
    ...
    let label = if with_residual {
        "fused_rmsnorm_residual (with residual)"
    } else {
        "fused_rmsnorm (no residual)"
    };
    print_phases(
        label,
        &[
...
unsafe fn bench_qknorm(head_dim: i32, n_q_heads: i32, n_k_heads: i32) {
    ...
    launch_rmsnorm_qk_pair_bf16_clocked(
        q_in as *const c_void,
        q_w as *const c_void,
        q_out as *mut c_void,
        k_in as *const c_void,
        k_w as *const c_void,
        k_out as *mut c_void,
        clocks as *mut c_void,
        head_dim,
    ...
    print_phases(
        &format!(
            "rmsnorm_qk_pair (n_q={}, n_k={}, head_dim={})",
            n_q_heads, n_k_heads, head_dim
        ),
        &[
            ("sumsq pass", load),
            ("reduction", reduce),
...
unsafe fn bench_turbo_reshape_and_cache(
    num_tokens: i32,
    num_heads: i32,
    head_size: i32,
    block_size: i32,
    ...
    cudaMemcpy(
        slots as *mut c_void,
        slots_h.as_ptr() as *const c_void,
        slots_n * 8,
        KIND_H2D,
    );
    ...
    turbo_reshape_and_cache_clocked(
        ...
        vn as *mut c_void,
        slots as *const i64,
        clocks_k as *mut c_void,
        clocks_v as *mut c_void,
        num_tokens,
        num_heads,
        head_size,
        block_size,
...
unsafe fn bench_tq_attn(
    ...
    cudaMemset(
        kn as *mut c_void,
        0,
        total_blocks * num_kv_heads as usize * block_size as usize * 2,
    );
    cudaMemset(
        vn as *mut c_void,
        0,
    ...
    cudaMemcpy(
        bt as *mut c_void,
        bt_h.as_ptr() as *const c_void,
        (num_seqs * mbps) as usize * 4,
        KIND_H2D,
    );
    ...
    print_phases(
        &format!("tq_attn (ctx={})", context_len),
        &[
...
fn main() {
    unsafe {
        ...
        println!(
            "{:>8}  {:>10}  {:>9}  {:>8}  {:>9}",
            "shape", "variant", "median μs", "GB/s", "% peak"
        );
        for &s in &shapes {
            ...
            println!(
                ...
                med_wide,
                achieved_wide,
                achieved_wide / 8000.0 * 100.0
            );
            println!(
                "{:>8}  {:>10}  best={} ({:.2}μs)",
                s.name, "→", best_name, best_med
            );
            println!();
