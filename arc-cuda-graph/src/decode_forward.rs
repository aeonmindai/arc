...
extern "C" {
    ...
    fn arc_launch_gemv_bf16_dual(
        weight_a: *const std::ffi::c_void,
        weight_b: *const std::ffi::c_void,
        input: *const std::ffi::c_void,
        out_a: *mut std::ffi::c_void,
        out_b: *mut std::ffi::c_void,
        m_a: i32,
        m_b: i32,
        k: i32,
        stream: CUstream,
    );
    fn arc_launch_gemv_bf16_dual_silu_mul(
        weight_gate: *const std::ffi::c_void,
        weight_up: *const std::ffi::c_void,
        input: *const std::ffi::c_void,
        output_mlp_act: *mut std::ffi::c_void,
        m: i32,
        k: i32,
        stream: CUstream,
    );
...
pub unsafe fn profile_forward(
    ...
    macro_rules! ms {
        ($a:expr, $b:expr) => {{
            ...
            cudaEventElapsedTime(&mut t, $a, $b);
            t
        }};
    }
    ...
    for layer_idx in 0..cfg.num_layers {
        ...
        launch_gather_rope_decode_bf16(
            q_ptr as *mut _,
            k_ptr as *mut _,
            buffers.cos_table as *const _,
            buffers.sin_table as *const _,
        ...
        if paged_attn.is_turbo {
            turbo_reshape_and_cache(
                ...
                lc.k_norms as *const _,
                lc.v_norms as *const _,
                ...
                paged_attn.norm_block_stride,
                paged_attn.norm_head_stride,
                stream,
                1,
            );
            if prof {
            ...
            turbo_paged_attention_v1(
                buffers.attn_out_f32 as *const _,
                q_ptr as *const _,
                lc.key_cache as *const _,
                lc.value_cache as *const _,
                lc.k_norms as *const _,
            ...
            launch_cast_f32_to_bf16(
                buffers.attn_out_f32 as *const _,
                buffers.attn_out as *mut _,
                (bs as usize * nh * hd) as i32,
                stream,
            );
            if prof {
                rec!(e_attn_cast);
        ...
        launch_residual_add_bf16(
            h_in as *const _,
            buffers.o_proj_out as *const _,
            buffers.residual as *mut _,
            (bs * hs) as i32,
            stream,
        );
        if prof {
    ...
    arc_launch_gemv_bf16_f32out(
        weights.lm_head.ptr as *const _,
...
pub unsafe fn decode_forward(
    ...
    for layer_idx in 0..cfg.num_layers {
        ...
        let residual_ptr = if deferred_residual != 0 {
            deferred_residual as *const _
        } else {
            std::ptr::null()
        };
        let residual_out_ptr = if deferred_residual != 0 {
            buffers.residual as *mut _
        } else {
            std::ptr::null_mut()
        };
        ...
        // MLP: fused gate+up+silu+mul GEMV — one kernel computes both
        // gate[m] and up[m] in registers, applies silu*mul, writes mlp_act
        // directly. Eliminates the standalone silu_mul launch and the
        // gate/up buffer round-trips.
        arc_launch_gemv_bf16_dual_silu_mul(
            lw.gate_proj.ptr as *const _,
            lw.up_proj.ptr as *const _,
            buffers.normed as *const _,
            buffers.mlp_act as *mut _,
            inter_z as i32,
            hs_z as i32,
...
extern "C" {
    fn launch_gather_embedding_bf16(
        embed_table: *const std::ffi::c_void,
        token_ids: *const i32,
        output: *mut std::ffi::c_void,
        hidden_size: i32,
        batch_size: i32,
    ...
    fn launch_rmsnorm_qk_pair_bf16(
        q_in: *const std::ffi::c_void,
        q_w: *const std::ffi::c_void,
        q_out: *mut std::ffi::c_void,
        k_in: *const std::ffi::c_void,
        k_w: *const std::ffi::c_void,
