...
//! Two C entry points:
//!
//! - `arc_flashmlasparse_score_bf16` — Per (B, H, T_q) indexer scoring:
//!   `scores[b,h,q,c] = dot(q[b,h,q,:], k[b,h,c,:]) * per_head_scale[b,h,q]`.
//!   FP32 output for the top-k consumer.
//! - `arc_flashmlasparse_topk` — Per (B*H*T_q) radix top-k selecting `topk`
...
//! and exposes a single `forward(q, k, scale) -> top_k_indices` call that
...
extern "C" {
    ...
    /// FP32 indexer scoring kernel (parity-test path).
    ///
    /// Same signature semantics as the BF16 variant; inputs and `per_head_scale`
    /// are FP32 instead of BF16. Used by the CPU-reference agreement test.
    pub fn arc_flashmlasparse_score_f32(
        q_ptr: *const c_void,
        k_ptr: *const c_void,
        scale_ptr: *const c_void,
        scores_ptr: *mut c_void,
