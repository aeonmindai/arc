...
mod tests {
    ...
    /// old tile-fill gate.
    ///
    /// This branch originally asserted against its own replacement predicate,
    /// `gather_policy::grouped_gemm_preferred` — a byte-traffic model gated on
    /// `GROUPED_MIN_PAIRS_PER_EXPERT = 1.09`. That predicate is not what ships:
    /// #197 arrived first and retired the tile-fill gate outright rather than
    /// loosening it, leaving [`prefer_gather_gemv`] — the GEMV arm below
    /// [`DECODE_REGIME_MAX_TOKENS`], grouped above it — as the whole dispatch.
    /// The two agree exactly at V4's top-6-of-256 (1.09 pairs per woken expert
    /// flips at n = 9; the floor is n > 8), so the boundary this test was
    /// written to pin is unchanged; only the predicate naming it is. Asserting
    /// against the shipped one is the point of the test.
    #[test]
    fn default_gate_selects_the_flat_arm_above_the_floor() {
        use crate::qtip::gather_policy::prefer_gather_gemv;
        // At and below the floor the GEMV is mandatory (CUDA-graph capture
        // safety) and measured a wash, so nothing is lost.
        assert!(prefer_gather_gemv(8, None));
