    ...
    fn gather_forward(&self, a: &Tensor, indices: &Tensor) -> Result<Tensor> {
        ...
        #[cfg(feature = "cuda")]
        {
                ...
                && matches!(a.dtype(), DType::BF16 | DType::F16 | DType::F32)
            {
                ...
                // on-device path enabled (the default). #197 was cut before
                // #212 and still had the presence-check `std::env::var(..)
                // .is_ok()` here; keeping #212's parser is deliberate, so this
                // retraction does not re-break the flag polarity.
