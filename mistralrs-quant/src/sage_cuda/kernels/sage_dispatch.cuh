 ...
 * dependency removed; this is the torch-free counterpart of dispatch_utils.h.
 *
...
#define DISPATCH_RETURN_LSE(return_lse, RETURN_LSE, ...)             \
  if (return_lse == 1) {                                         \
    ...
    std::ostringstream err_msg;                                  \
    err_msg << "Unsupported return_lse mode: " << int(return_lse);   \
    throw std::invalid_argument(err_msg.str());                  \
  }

// out_dtype: 0 = half (float16), 1 = nv_bfloat16
#define DISPATCH_OUT_DTYPE(out_dtype, c_type, ...)                          \
  if ((out_dtype) == 0) {                                                   \
    using c_type = half;                                                    \
    __VA_ARGS__                                                             \
  } else if ((out_dtype) == 1) {                                            \
    ...
    std::ostringstream err_msg;                                             \
    err_msg << "Unsupported output dtype: " << int(out_dtype);              \
    throw std::invalid_argument(err_msg.str());                             \
  }
